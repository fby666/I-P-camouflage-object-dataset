...
class GNet(nn.Module):
    def __init__(self, norm_layer = nn.LayerNorm):
        ...
        self.I_swin = SwinTransformer(embed_dim=128, depths=[2,2,18,2], num_heads=[4,8,16,32])
        self.P_swin = SwinTransformer(embed_dim=128, depths=[2,2,18,2], num_heads=[4,8,16,32])
        self.fuse = fuse_enhance3(64)
        self.GFF1 = GFF(channel=512)
        self.GFF2 = GFF(channel=256)
        self.GFF3 = GFF(channel=128)

        self.up2 = nn.UpsamplingBilinear2d(scale_factor = 2)
        self.up4 = nn.UpsamplingBilinear2d(scale_factor = 4)
        self.up8 = nn.UpsamplingBilinear2d(scale_factor = 8)
        self.up16 = nn.UpsamplingBilinear2d(scale_factor = 16)
        self.conv2048_1024 = conv3x3_bn_relu(2048,64)

        self.conv1024_512 = conv3x3_bn_relu(128, 64)
        self.conv512_256 = conv3x3_bn_relu(128, 64)
        self.conv256_32 = conv3x3_bn_relu(128, 64)
    ...
    def forward(self,x ,d):
        I_list = self.I_swin(x)
        P_list = self.P_swin(d)
        ...
        fuse_fea1 = self.fuse(r1, d1)

        fuse_fea1 = self.up2(fuse_fea1)

        fr2, fd2,fuse_fea1 = self.GFF1(r2, d2,fuse_fea1)
        ...
        fr3, fd3,fuse_fea2 = self.GFF2(r3, d3,fuse_fea2)
        ...
        fr4, fd4, fuse_fea3 = self.GFF3(r4, d4, fuse_fea3)

        mul_fea4 = fr4 * fd4
        add_fea4 = fr4 + fd4
        fuse_fea4 = torch.cat((mul_fea4, add_fea4), dim=1)
        fuse_fea4 = self.conv256_32(fuse_fea4)
        fuse_fea4 = fuse_fea4 + fuse_fea3

        del d4, d3, d2

        end_sal = self.conv64_32(fuse_fea4)
...
class fuse_enhance1(nn.Module):
    def __init__(self, infeature):
        super(fuse_enhance1, self).__init__()
        self.depth_channel_attention = CA_Block(infeature)
        self.rgb_channel_attention = CA_Block(infeature)
        self.rd_spatial_attention = SA_Block(infeature)
...
#
    def forward(self,r,d):
        ...
        d_out = d * d_ca
        return r_out, d_out
class fuse_enhance3(nn.Module):
    def __init__(self, infeature):
        super(fuse_enhance3, self).__init__()
       # self.depth_channel_attention = CA_Block(infeature)
        self.channel_attention = SKConv(features=infeature, M=2,  r=2, L=32)

        self.spatial_attention = SA_Block(infeature)

    def forward(self,r,d):
        assert r.shape == d.shape,"rgb and depth should have same size"
        mul_fuse=self.channel_attention(r,d)

        out=self.spatial_attention(mul_fuse)
...
class fuse_enhance(nn.Module):
    ...
    def forward(self,r,d):
        ...
        return r_out, d_out,sa
class fuse_enhance4(nn.Module):
    def __init__(self, infeature,up):
        super(fuse_enhance4, self).__init__()

        self.up2 = nn.UpsamplingBilinear2d(scale_factor=up)
        self.depth_channel_attention = ChannelAttention(infeature)
        self.rgb_channel_attention = ChannelAttention(infeature)

    def forward(self,r,d,sa):
        assert r.shape == d.shape,"rgb and depth should have same size"
        sa=self.up2(sa)
...
class SKConv(nn.Module):
    def __init__(self, features, M,  r=2, L=32):
        super(SKConv, self).__init__()
        d = max(int(features / r), L)
        self.M = M
        self.features = features
        self.conv1024_64_r = nn.Sequential(
                    nn.Conv2d(1024,
                              64,
                              kernel_size=5,
                              stride=1,
        ...
        self.conv1024_64_d = nn.Sequential(
                    nn.Conv2d(1024,
                              64,
                              kernel_size=3 ,
                              stride=1,
   ...
   #             nn.ReLU(inplace=False)))
        ...
        self.fcs1=nn.Sequential(nn.Linear(d, features),
                                nn.Softmax())
        self.fcs2=nn.Sequential(nn.Linear(d,features),
                                nn.Softmax())
    ...
    def forward(self, x,b):
        ...
        x = self.conv1024_64_r(x)
        #x1=x
        b = self.conv1024_64_d(b)

        fea_U =x+b

        fea_s = fea_U.mean(-1).mean(-1)#32*256


        fea_z = self.fc(fea_s)
        f1=self.fcs1(fea_z).unsqueeze(-1).unsqueeze(-1)
        f2=self.fcs2(fea_z).unsqueeze(-1).unsqueeze(-1)
        x=f1*x
        b=f2*b
        fea_v=x+b
